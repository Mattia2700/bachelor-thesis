...
The map currently in use was created using \acrshort{slam}, and since it is no longer possible to graphically control the robot with \acrshort{rviz} when doing so, another way must be found: the solution is the \code{joy} driver package, which makes an \textbf{Xbox controller} capable of setting wheels velocity over \code{cmd\_vel} topic (via \textbf{Bluetooth}).
