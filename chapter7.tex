...
\item Talking about the navigation part, the \textbf{obstacle avoidance} could be enhanced, especially in the case of \textbf{dynamic obstacles}, because the robot sometimes takes some time to understand what is happening, and may end up \textbf{colliding} with them. A problem could also be the \code{ros1\_bridge} that adds some delay in the exchange of messages between \acrshort{ros}1 and \acrshort{ros}2, ending up receiving lidar data not instantly; rewrite \acrshort{ros}1 nodes in \acrshort{ros}2 nodes may resolve this issue, by getting rid of the bridge.
